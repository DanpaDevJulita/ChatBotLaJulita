...
export async function consultarDisponibilidadPorDia(
  ...
  try {
    const res = await axios.get(`${API_OFICIAL_URL}/available-rooms`, {
      params: { api_token: API_TOKEN, start_date: fechaEntradaISO, end_date: fechaSalidaISO },
      timeout: TIMEOUT_MS,
      headers: { Accept: "application/json" },
    });

    // Forma esperada: [{ date: "2026-12-19", name: "CHALET", available_rooms: 1 }, ...]
    const filas: unknown = Array.isArray(res.data) ? res.data : (res.data as any)?.data;
    if (!Array.isArray(filas)) {
      ultimoEstadoOficial = "respuesta_inesperada";
      console.error("[lobbypms] la API oficial no devolvió un arreglo de disponibilidad —", explicar("respuesta_inesperada"));
      return null;
    }
    ...
    for (const fila of filas as any[]) {
      ...
      const nombre = String(fila?.name ?? "").trim().toUpperCase();
      if (!/^\d{4}-\d{2}-\d{2}$/.test(fecha)) continue;
      const mapeo = CATEGORIAS_LOBBY[nombre];
      if (!mapeo) {
        console.warn(`[lobbypms] categoría sin mapear: "${fila?.name}" — revisar CATEGORIAS_LOBBY`);
        continue;
      }
      if (!porFecha.has(fecha)) porFecha.set(fecha, []);
      porFecha.get(fecha)!.push({
        nombreLobby: String(fila.name),
        clase: mapeo.clase,
        capacidad: mapeo.capacidad,
        disponibles: Number(fila?.available_rooms ?? 0),
      });
    }
 ...
 */
function agregarPorRango(dias: DisponibilidadDia[]): DisponibilidadCategoria[] {
 ...
 * (src/core/tools/catalogo.ts) la sigue usando igual.
 */
export async function consultarDisponibilidad(
  ...
  if (porDia && porDia.length > 0) return agregarPorRango(porDia);
...
export async function buscarFechasAlternativas(
  ...
  for (let i = 0; i <= dias; i++) {
    ...
    const conCupo = agregarPorRango(nochesDeLaEstadia).filter((c) => c.disponibles > 0);
